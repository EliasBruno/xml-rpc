...
public class ClientDistribuition extends Thread{
   ...
   public void run() {
	   ...
	   switch (this.server) {
		case 1:
		    ...
		    this.sendServer(init, end, 8081);
		    populate(vector);
		    break;
		case 2:
			init = (tam / 4) * 1;
			end = (tam / 4) * 2;
		    this.sendServer(init, end, 8082);
		    populate(vector);
		    break;
		case 3:
			init = (tam / 4) * 2;
			end = (tam / 4) * 3;
		    this.sendServer(init, end, 8083);
		    populate(vector);
		    break;
		case 4:
			init = (tam / 4) * 3;
			end = tam;
		    this.sendServer(init, end, 8084);
		    populate(vector);
		    break;
		default:
			break;
   ...
   public void sendServer(int init, int end, int port) throws XmlRpcException, IOException {
   ...
   public static void populate(Vector vetor) throws IOException {
	   for (int i = 0; i<vetor.size(); i++) {
	       ...
	       image.setRGB(Integer.parseInt(new_vector[0]), Integer.parseInt(new_vector[1]), Integer.parseInt(new_vector[2]));
	   }  
   }
}